 ...
 * If not configured or module unavailable, falls back to console.log for safety.
 */
let client = null;
try {
  // Gracefully handle missing twilio module in production builds.
  // Avoid static bundler tracing by using eval('require') instead of a plain require call.
  if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
    const twilioRequire = eval('require');
    const twilioLib = twilioRequire('twilio');
    const Twilio = twilioLib?.Twilio;
    if (Twilio) {
      client = new Twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
    }
  }
} catch (e) {
  // If twilio isn't installed or environment is incomplete, fallback safely.
  client = null;
}
